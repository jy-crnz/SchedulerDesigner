document.addEventListener('DOMContentLoaded', () => {
    ...
    let isDragging = false; // Add this at the very top with your other variables
    ...
    const openModal = (target, type) => {
        ...
        if (type === 'header') {
            ...
            // FIX: Show star button if there is no subject, OR if there is already a star
    ...
    modalApply.onclick = () => {
        if (modalTitle.innerText === "Edit Header") {
            ...
            if (s) {
                // Keep the handle when updating content
    ...
    // FIX: Preserve Handle on Star Toggle
    modalStar.onclick = () => {
        const handleHTML = `<div class="drag-handle"><span></span><span></span><span></span><span></span><span></span><span></span></div>`;
...
function initGrid() {
    ...
    // DEBUG: Create a small status box if it doesn't exist
    ...
    for (let i = 0; i < 25; i++) {
        ...
        cell.addEventListener('dragover', (e) => {
            ...
            // Look 'through' the ghost image to see what cell is underneath
            ...
            if (target && !target.classList.contains('day-label')) {
                ...
                // Update your debug box if you still have it
                const debugBox = document.getElementById('mobile-debug-status');
        ...
        cell.addEventListener('drop', (e) => {
            ...
            if (sourceIndex !== null && targetCell && targetCell.id !== `cell-${sourceIndex}`) {
    ...
    function saveToLocal() {
        ...
        const headerData = {
            start: yearStartInput.value, end: yearEndInput.value,
            term: termInput.value, theme: wpContainer.className
        };
    ...
    downloadBtn.onclick = () => {
    // 1. Temporarily add the 'exporting' class to hide UI icons
    ...
    html2canvas(wpContainer, { 
        ...
        // 2. Remove the class immediately after the screenshot is taken
    ...
    function clearInputs() { subInput.value = ''; timeInput.value = ''; roomInput.value = ''; }
